...
// a function that gets the user_id, course_id, paypal_capture_id
// and register the user to course in DB.
// receipt_number is an auto increment column, so it is not passed in — it is
// generated by the DB and comes back as result.insertId
function registerUserToCourse(user_id, course_id, paypal_capture_id, cb) {
  const conn = db.getConnection();
...
// the PayPal capture id is stored in paypal_capture_id — receipt_number is the
// auto increment receipt the DB generates, and it is passed back to the caller
